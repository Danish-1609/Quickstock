      ...
      {showKeyInput && (
        <div style={{ background: "#FFF3CD", padding: "12px 20px", borderBottom: "1px solid #FFE082" }}>
          <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            <span style={{ fontSize: 13, color: "#7A5F00" }}>🔑 Enter Anthropic API Key to use AI:</span>
            <input
              type="password"
              placeholder="sk-ant-..."
              value={apiKey}
              ...
              <div style={{ background: "#FFF0EE", border: "1px solid #FFCDD2", borderRadius: 14, padding: 16 }}>
                ...
                {criticalItems.map(p => (
                  <div key={p.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderTop: "1px solid #FFCDD2" }}>
                    ...
                    <span style={{ fontSize: 13, color: "#D93025", fontWeight: 700 }}>
                      {daysUntilStockout(p.stock, p.dailySales)} days left
                    </span>
                  </div>
                ))}
              </div>
          ...
          <div>
            {!forecast && !loading && (
              <div style={{ textAlign: "center", padding: "60px 20px", color: "#6B6B6B" }}>
                ...
                <h3 style={{ fontFamily: "'Baloo 2', cursive", fontSize: 20, marginBottom: 8, color: "#1C1C1C" }}>
                  AI Forecast Not Run Yet
                </h3>
            ...
            {forecast && (
              <div>
                <div style={{ background: "linear-gradient(135deg, #FF6B00, #FF8C38)", borderRadius: 14, padding: 20, color: "#fff", marginBottom: 20 }}>
                  ...
                  {forecast.festivalNote && (
                    <p style={{ fontSize: 13, marginTop: 10, opacity: 0.9 }}>🎉 {forecast.festivalNote}</p>
                  )}
                </div>
                    ...
                    <div key={i} style={{
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 8 }}>
                        <div style={{ flex: 1 }}>
                          ...
                          {item.festivalAlert && (
                            <div style={{ fontSize: 12, color: "#FF6B00", marginTop: 4 }}>🎉 {item.festivalAlert}</div>
                          )}
                        </div>
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
                          ...
                          {item.reorderQty > 0 && (
                            <span style={{ fontSize: 13, color: "#1A7A4A", fontWeight: 600 }}>
                              Order: {item.reorderQty} units
                            </span>
                          )}
                          {item.revenueRisk > 0 && (
                            <span style={{ fontSize: 12, color: "#7B1FA2" }}>
                              Risk: ₹{item.revenueRisk.toLocaleString("en-IN")}
                            </span>
                          )}
                        </div>
                      </div>
